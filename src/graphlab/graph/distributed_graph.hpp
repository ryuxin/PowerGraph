...
namespace graphlab {
  ...
  public:
    ...
    /// The type of the edge data stored in the graph
    typedef GatherData   gather_data_type;
    ...
    typedef graphlab::dynamic_local_graph<VertexData, EdgeData, GatherData, Graph_alloctor> local_graph_type;
#else 
    typedef graphlab::local_graph<VertexData, EdgeData> local_graph_type;
#endif
    typedef graphlab::distributed_graph<VertexData, EdgeData, GatherData, Graph_alloctor> graph_type;

    typedef std::vector<simple_spinlock> lock_manager_type;

    friend class distributed_ingress_base<VertexData, EdgeData, GatherData, Graph_alloctor>;
...
#pragma omp parallel
    ...
    void save_to_posixfs(const std::string& prefix, Writer writer,
      ...
      for(size_t i = 0; i < graph_files.size(); ++i) {
        ...
        typedef distributed_graph<vertex_data_type, edge_data_type, GatherData, Graph_alloctor> graph_type;
        vertex_callbacks[i] =
...
#pragma omp parallel for
  ...
  private:
    ...
    distributed_ingress_base<VertexData, EdgeData, GatherData, Graph_alloctor>* ingress_ptr;
    ...
    void set_ingress_method(const std::string& method,
      ...
      if (method == "oblivious") {
        ...
        ingress_ptr = new distributed_oblivious_ingress<VertexData, EdgeData, GatherData, Graph_alloctor>(rpc.dc(), *this, usehash, userecent);
      } else if (method == "hdrf") {
        if (rpc.procid() == 0) logstream(LOG_EMPH) << "Use hdrf oblivious ingress, usehash: " << usehash
          << ", userecent: " << userecent << std::endl;
        ingress_ptr = new distributed_hdrf_ingress<VertexData, EdgeData, GatherData, Graph_alloctor>(rpc.dc(), *this, usehash, userecent);
      } else if  (method == "random") {
        if (rpc.procid() == 0)logstream(LOG_EMPH) << "Use random ingress" << std::endl;
        ingress_ptr = new distributed_random_ingress<VertexData, EdgeData, GatherData, Graph_alloctor>(rpc.dc(), *this);
      } else if (method == "grid") {
        if (rpc.procid() == 0)logstream(LOG_EMPH) << "Use grid ingress" << std::endl;
        ingress_ptr = new distributed_constrained_random_ingress<VertexData, EdgeData, GatherData, Graph_alloctor>(rpc.dc(), *this, "grid");
      } else if (method == "pds") {
        if (rpc.procid() == 0)logstream(LOG_EMPH) << "Use pds ingress" << std::endl;
        ingress_ptr = new distributed_constrained_random_ingress<VertexData, EdgeData, GatherData, Graph_alloctor>(rpc.dc(), *this, "pds");
      } else {
